...
def check(kk):
    ...
    for _ in range(len(kk)):
        opcode = int(str(kk[instruction])[-2:])
        modes = str(kk[instruction])[:-2]
        if opcode == 1:
            kk[kk[instruction + output]] = codes(kk, instruction, modes, 1) + codes(kk, instruction, modes, 2)
            instruction += 4
        elif opcode == 2:
            kk[kk[instruction + output]] = codes(kk, instruction, modes, 1) * codes(kk, instruction, modes, 2)
            instruction += 4
        elif opcode == 3:
            kk[kk[instruction + input1]] = 5 #or 1 as user input
            instruction += 2
        elif opcode == 4:
            print(codes(kk, instruction, modes, 1))
            instruction += 2
        elif opcode == 5:
            if codes(kk, instruction, modes, 1) != 0:
                instruction = codes(kk, instruction, modes, 2)
            else:
                instruction += 3
        elif opcode == 6:
            if codes(kk, instruction, modes, 1) == 0:
            ...
            kk[kk[instruction + output]] = 1 if codes(kk,instruction, modes, 1) < codes(kk, instruction, modes, 2) else 0
            instruction += 4
        elif opcode == 8:
            kk[kk[instruction + output]] = 1 if codes(kk,instruction, modes, 1) == codes(kk, instruction, modes, 2) else 0
            instruction += 4
            ...
            break
# 1st solution - 16574641
# ---------------
# 2nd solution - 15163975
# ---------------
check(ll)